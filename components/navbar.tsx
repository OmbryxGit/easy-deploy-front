...
import { CircleCheck as CircleCheckIcon, Circle as CircleHelpIcon, Circle as CircleIcon, User as UserIcon, Menu as MenuIcon, Bone as XIcon } from "lucide-react"
...
export function Navbar() {
    ...
    return (
        <>
        <div className="fixed top-5 w-full flex items-center z-50 px-4 sm:px-6 lg:px-8">

        <header className="w-full max-w-7xl border-b min-h-[8vh] flex items-center justify-between px-4 sm:px-6 lg:px-8 shadow-sm shadow-secondary/50 rounded-xl mx-auto bg-background/70 backdrop-blur-sm">
            <Link href={"/"} className="flex-shrink-0">

            <section className="flex items-center gap-2">
                <svg width="960" height="683" className="h-8 w-8 sm:h-10 sm:w-10" viewBox="0 0 960 683" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <h1 className="text-lg sm:text-xl lg:text-2xl font-bold whitespace-nowrap">Easy <span className="text-secondary">Deploy</span></h1>
            </section>
            </Link>
            ...
            <div className="flex items-center gap-2">
                <ModeToggle/>
                <div className="hidden sm:flex h-10 w-10 bg-secondary/50 rounded-full items-center justify-center">
                    <UserIcon size={20} />
                </div>
                ...
                >
                    {isMobileMenuOpen ? <IoMdClose size={24}  /> : <MenuIcon size={24} />}
                </button>
            </div>
        </header>
        </div>
        ...
        {isMobileMenuOpen && (
            <div className="fixed inset-0 z-40 lg:hidden">
                ...
                />
                <nav className="fixed top-[12vh] left-4 right-4 bg-background/95 backdrop-blur-md border rounded-xl shadow-lg p-4 animate-in slide-in-from-top-4 duration-200">
                    <div className="flex flex-col space-y-1">
                        {navItems.map((item) => (
                            <Link
                                ...
                                href={item.href}
                                onClick={() => setIsMobileMenuOpen(false)}
                                className="px-4 py-3 text-base font-medium text-gray-700 dark:text-gray-300 hover:text-[#9333EA] dark:hover:text-[#9333EA] transition-all duration-200 rounded-lg hover:bg-gradient-to-r hover:from-[#9333EA]/5 hover:to-[#2563EB]/5"
                            >
                                {item.label}
                            </Link>
                        ))}
                        <div className="sm:hidden pt-2 border-t mt-2">
                            <Link
                                href="/profile"
                                onClick={() => setIsMobileMenuOpen(false)}
                                className="flex items-center gap-2 px-4 py-3 text-base font-medium text-gray-700 dark:text-gray-300 hover:text-[#9333EA] dark:hover:text-[#9333EA] transition-all duration-200 rounded-lg hover:bg-gradient-to-r hover:from-[#9333EA]/5 hover:to-[#2563EB]/5"
                            >
                                <UserIcon size={20} />
                                Profil
                            </Link>
